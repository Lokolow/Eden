...
void EmulationSession::InitializeSystem(bool reload) {
    if (!reload) {
        ...
        if (Settings::values.enable_thermal_throttle.GetValue()) {
            auto device_model = AndroidThermal::DetectDeviceModel();
            m_thermal_monitor = std::make_unique<AndroidThermal::ThermalMonitor>();
            m_thermal_monitor->Initialize(device_model);
            
            // Set callbacks for thermal events
            m_thermal_monitor->SetOnCriticalCallback([this]() {
                LOG_CRITICAL(Frontend, "🔥 CRITICAL TEMPERATURE - Pausing emulation!");
                PauseEmulation();
            });
            
            m_thermal_monitor->SetOnEmergencyCallback([this]() {
                LOG_CRITICAL(Frontend, "🚨 EMERGENCY - Stopping emulation!");
                HaltEmulation();
            });
            
            m_thermal_monitor->StartMonitoring();
            LOG_INFO(Frontend, "Thermal protection initialized for device model: {}", 
                     static_cast<int>(device_model));
        }
    }
...
void EmulationSession::ShutdownEmulation() {
    ...
    if (m_thermal_monitor) {
        m_thermal_monitor->StopMonitoring();
        m_thermal_monitor.reset();
    }
