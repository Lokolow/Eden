...
#include "video_core/opengl_frame_generator.h"
...
RendererOpenGL::RendererOpenGL(Core::Frontend::EmuWindow& emu_window_,
    ...
    // Initialize OpenGL Frame Generator
    if (Settings::values.frame_interpolation.GetValue()) {
        frame_generator = std::make_unique<VideoCore::OpenGLFrameGenerator>();
        frame_generator->Initialize(1280, 720);
        frame_generator->Enable(true);
